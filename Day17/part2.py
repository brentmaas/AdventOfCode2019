...
m = "C,A,C,B,A,B,A,B,C,B\n"
a = "L,8,R,10,R,6\n"
b = "R,12,L,10,R,10,L,8\n"
c = "R,12,L,10,R,12\n"
stdin = io.StringIO(" ".join([str(ord(i)) for i in m]) + "\n" + " ".join([str(ord(i)) for i in a]) + "\n" + " ".join([str(ord(i)) for i in b]) + "\n" + " ".join([str(ord(i)) for i in c]) + "\n" + str(ord("n")) + " " + str(ord("\n")) + "\n")
stdout = io.StringIO("")
computer(program + [0] * 10000, stdin=stdin, stdout=stdout)
print(stdout.getvalue().split("\n")[-2])