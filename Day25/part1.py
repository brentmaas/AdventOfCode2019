...
while True:
	stdout = io.StringIO("")
	state = computer(program, stdin=stdin, stdout=stdout, returnonemptyread=True, state=state[0], base_state=state[1])
	print("".join([chr(int(i)) for i in stdout.getvalue()[:-1].split("\n")]))
	inp = input()
	stdin = io.StringIO(" ".join([str(ord(i)) for i in inp]) + " 10")